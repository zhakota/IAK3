...
```{r}
df3 %>% 
select(Case_Num, Age, Gender, Height, Weight, NYHA_Heart_Fail_Class, Prosthesis_Anat_Size, Upsizing, Pre_LVOT) %>% 
diagnose_numeric()
...
```{r}
df3 %>% 
select(Case_Num, Age, Gender, Height, Weight, NYHA_Heart_Fail_Class, Prosthesis_Anat_Size, Pre_LVOT) -> df3d
...
Prosthesis_Anat_Size3m <- imputate_na(df3d, Prosthesis_Anat_Size, Pre_LVOT, method = "mice", seed = 123) #Multivariate Imputation by Chained Equations
...
df3 %>% 
select(Case_Num, Age, Gender, Height, Weight, NYHA_Heart_Fail_Class, Prosthesis_Anat_Size, Pre_LVOT) -> df4

# Imputation methods
imp_pmm <- mice(df4, meth = "pmm", seed = 123) # Predictive Mean Matching
...
imp_cart <- mice(df4, meth = "cart", seed = 123) # Classification and Regression Trees
imp_rf <- mice(df4, meth = "rf", seed = 123) # Random Forest  
imp_mean <- mice(df4, meth = "mean", seed = 123) # Mean
imp_norm <- mice(df4, meth = "norm", seed = 123) # Bayesian linear regression
...
mice::densityplot(imp_pmm)
mice::densityplot(imp_midastouch)
mice::densityplot(imp_sample)
mice::densityplot(imp_cart)
mice::densityplot(imp_rf)
mice::densityplot(imp_mean)
mice::densityplot(imp_norm)
# mice::densityplot(imp_norm_nob)
# mice::densityplot(imp_norm_boot)
# mice::densityplot(imp_norm_predict)
mice::densityplot(imp_lasso.norm)
mice::densityplot(imp_lasso.select.norm)
# mice::densityplot(imp_quadratic)
# mice::densityplot(imp_ridge)

mice::densityplot(imp_pmm, imp_cart, imp_rf, imp_mean)
...
#####################################################################################################################
fit <- with(df, lm(Prosthesis_Anat_Size ~ Age + Gender + Height + Weight + NYHA_Heart_Fail_Class + Pre_LVOT))
...
fit2 <- with(df_rf, lm(Prosthesis_Anat_Size ~ Age + Gender + Height + Weight + NYHA_Heart_Fail_Class + Pre_LVOT))
# summary(pool(fit))
summary(fit2)
...
MissRander_df3 <- missRanger(
  df3, 
  formula = Prosthesis_Anat_Size ~ c(Age, Gender, Height, Weight, NYHA_Heart_Fail_Class, Prosthesis_Anat_Size, Pre_LVOT),
  pmm.k = 3,
  num.trees = 1000, 
  verbose = 2, 
  seed = 123,  
...
# Imputation for Prosthesis_Anat_Size only
...
ggplot()+
  geom_point(data = MissRander_df30$data, aes(Prosthesis_Anat_Size, Pre_LVOT), color = "red")+
...
# Сравние группы original и imputation по размеру протеза. Вменение было только для размеров от 21 до 26. Сравниваем для каждого размера группы original и imputation.
 grouped_ggbetweenstats(
  data = Prosthesis_Anat_Size_compare,
...
ggscatterstats(
  ...
  y = Pre_LVOT,
  title = "Связь между анатомическим и ЭхоКГ размерамим аортального клапана",
  xlab = "Анатомический диаметр клапана во время операции (мм)",
  ylab = "ЭхоКГ диаметр клапана (LVOT) до операции (мм)",
  # legend.title = "group",
  ggtheme = theme_minimal()
)  

ggscatterstats(
  data = MissRander_df30$data,
  x = Prosthesis_Anat_Size,
  y = Pre_LVOT,
  title = "Связь между анатомическим и ЭхоКГ размерамим аортального клапана",
  xlab = "Анатомический диаметр клапана во время операции (мм)",
...
MissRander_df3_merge %>% 
  select(Prosthesis_Size, Prosthesis_Anat_Size, Upsizing, Pre_LVOT) %>% 
  plot_na_pareto()
